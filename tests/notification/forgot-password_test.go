...
var _ = Describe("POST /notification/forgot-password", func() {
	var (
		email        string
		responseBody gin.H
	)
	...
	var ExecuteRequest = func() (*httptest.ResponseRecorder, error) {
		...
		request, err := http.NewRequest(http.MethodPost, "/notification/forgot-password", bytes.NewReader(requestBodyBytes))
		if err != nil {
			return nil, err
		}
